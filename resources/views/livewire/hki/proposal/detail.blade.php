        ...
        <div class="flex">
          ...
          <div class="ml-3">
            ...
            <div class="mt-2 text-sm text-orange-700">
              <p>Proposal HKI Anda memerlukan perbaikan. Silakan periksa catatan reviewer di bagian bawah halaman ini dan
                lakukan revisi yang diperlukan.</p>
            </div>
          </div>
        </div>
      </div>
    ...
    @can('review-hki')
      @if($proposal->status === 'SUBMITTED')
        <div class="mb-6 bg-white rounded-lg border border-gray-200 overflow-hidden">
          ...
          <div class="px-6 py-4">
            <p class="text-sm text-gray-700 mb-4">Silakan melakukan review terhadap kelengkapan dokumen dan berikan
              keputusan:</p>
    ...
    <div class="mb-6 bg-white rounded-lg border border-gray-200 overflow-hidden">
      ...
      <div class="p-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <dt class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Negara Pertama kali diumumkan
            </dt>
            @if($isEditMode)
          ...
          <div>
            <dt class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Tanggal Pertama kali diumumkan
            </dt>
            @if($isEditMode)
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
          <div class="bg-gray-50 px-6 py-4 border-b border-gray-200">
            <h2 class="text-lg font-semibold text-gray-900">Daftar Tim Pengusul ({{ $proposal->members->count() }}
              Anggota)</h2>
          </div>
          <div class="p-6">
                  ...
                  <div class="flex items-start justify-between mb-3">
                    <div class="flex-1">
                      <div class="flex items-center gap-2 flex-wrap mb-2">
                        ...
                        <span
                          class="px-2 py-0.5 inline-flex items-center text-xs leading-5 font-semibold rounded-full {{ $member->role == 'KETUA' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800' }}">
                          {{ $member->role }}
                        </span>
                      </div>
      ...
      <div class="col-span-1">
        <div class="bg-white rounded-lg border border-gray-200 overflow-hidden lg:sticky lg:top-6">
          <div class="bg-gray-50 px-6 py-4 border-b border-gray-200 flex items-center justify-between">
            ...
            <button wire:click="auditHashChain" wire:loading.attr="disabled"
              class="inline-flex items-center px-3 py-1 bg-blue-100 text-blue-700 text-xs font-semibold rounded-lg hover:bg-blue-200 focus:outline-none">
              <svg wire:loading.remove wire:target="auditHashChain" class="w-4 h-4 mr-1" fill="none"
                stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
              </svg>
              <svg wire:loading wire:target="auditHashChain" class="animate-spin h-4 w-4 mr-1 text-blue-700"
          ...
          <div class="p-6">
            @if($auditResults)
              <div
                class="mb-6 p-4 rounded-lg {{ $auditResults['is_valid'] ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200' }}">
                ...
                @if(!$auditResults['is_valid'] && count($auditResults['errors']) > 0)
                  <div class="mt-3 space-y-2">
                    ...
                    @endforeach
                    @if(count($auditResults['errors']) > 3)
                      <p class="text-[10px] text-red-500 italic">...dan {{ count($auditResults['errors']) - 3 }} kesalahan
                        lainnya.</p>
                    @endif
                  </div>
                @endif
                ...
                <div
                  ...
                  <div class="bg-white px-6 py-4">
                    <div class="space-y-4">
                      ...
                      @if($reviewDecision)
                        <div
                          class="bg-{{ $reviewDecision === 'approved' ? 'green' : ($reviewDecision === 'revision' ? 'orange' : 'red') }}-50 border border-{{ $reviewDecision === 'approved' ? 'green' : ($reviewDecision === 'revision' ? 'orange' : 'red') }}-200 rounded-lg p-3">
                          <p
                            class="text-xs text-{{ $reviewDecision === 'approved' ? 'green' : ($reviewDecision === 'revision' ? 'orange' : 'red') }}-800">
                            @if($reviewDecision === 'approved')
                              <strong>Persetujuan:</strong> Proposal akan disetujui dan status berubah menjadi APPROVED.
                            @elseif($reviewDecision === 'revision')
                              <strong>Perlu Revisi:</strong> Proposal akan dikembalikan ke pengusul untuk diperbaiki sesuai
                              catatan.
                            @else
