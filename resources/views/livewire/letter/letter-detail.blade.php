...
<div class="max-w-7xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="flex items-center justify-between p-3 border border-indigo-100 bg-indigo-50 rounded-lg">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <div class="text-xs text-gray-500">{{ number_format($doc->file_size / 1024, 0) }} KB • Uploaded {{ $doc->created_at->diffForHumans() }}</div>
                            </div>
                        </div>
                        <a href="{{ Storage::url($doc->file_path) }}" target="_blank" class="text-indigo-600 hover:text-indigo-800 text-sm font-bold underline">
                            Download
                        </a>
                    </div>
                @endforeach
            ...
            <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                <div class="flex justify-between items-center mb-4 border-b pb-2">
                    ...
                    <span class="text-lg font-bold text-indigo-600">Total: Rp {{ number_format($submission->total_budget_proposed) }}</span>
                </div>
        ...
        <div class="space-y-6">
            ...
            {{-- TOMBOL BACK --}}
            <a href="{{ route('grants.list') }}" class="block w-full text-center py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
                ...
                @if($submission->status === 'SUBMITTED')
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="number" wire:model="score" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-yellow-500 focus:ring-yellow-500 sm:text-sm">
                        ...
                        <div>
                            ...
                            <textarea wire:model="comment" rows="4" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-yellow-500 focus:ring-yellow-500 sm:text-sm" placeholder="Berikan masukan atau alasan penolakan..."></textarea>
                            @error('comment') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                        </div>

                        <div class="grid grid-cols-2 gap-3 pt-2">
                            <button wire:click="submitReview('REJECT')" wire:confirm="Yakin tolak proposal ini?" class="w-full py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-red-600 hover:bg-red-700 focus:outline-none">
                                Tolak 🚫
                            </button>
                            <button wire:click="submitReview('ACCEPT')" wire:confirm="Yakin setujui proposal ini?" class="w-full py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-green-600 hover:bg-green-700 focus:outline-none">
                                Setujui ✅
                            </button>
                        </div>
                    </div>
