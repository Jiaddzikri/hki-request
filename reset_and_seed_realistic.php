...
echo "Semua data proposal dan log lama telah dihapus bersih.\n\n";
...
$count = count($realisticTitles);
echo "Memulai pembuatan $count proposal realistis beserta Audit Log-nya...\n";
...
foreach ($realisticTitles as $index => $title) {
    ...
    // Karena kita sudah memperbarui AuditLogService, 'created_at' dan 'updated_at' akan ter-inject otomatis!
    ...
    $num = $index + 1;
    echo "[$num/$count] Berhasil Dibuat: {$proposal->title}\n";
}

echo "\nSelesai! $count Proposal realistis berhasil ditambahkan dengan perlindungan Hash Chain & Timestamp penuh.\n";
